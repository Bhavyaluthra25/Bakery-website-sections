...
function CakeCard({ cake, onAddToCart }: { cake: Cake; onAddToCart: (item: CartItem) => void }) {
  ...
  function handleAdd() {
    onAddToCart({ id: `${cake.id}-${selectedSize}`, name: cake.name, size: selectedSize, price: cake.prices[selectedSize], image: cake.image, qty: 1 });
    setAdded(true);
      ...
      <div style={{ padding: "16px 18px 20px", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {(["250g", "500g", "1kg"] as Size[]).map(size => (
            <button key={size} onClick={() => setSelectedSize(size)} style={{ flex: 1, padding: "8px 4px", borderRadius: 10, border: selectedSize === size ? "2px solid #c8873a" : "1px solid #e8e0d0", background: selectedSize === size ? "#fff8f0" : "#fafaf8", cursor: "pointer", transition: "all 0.15s" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "auto" }}>
          ...
          <button onClick={handleAdd} style={{ padding: "10px 18px", borderRadius: 10, border: "none", background: added ? "#4caf50" : "#c8873a", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "background 0.2s", whiteSpace: "nowrap" }}>
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          {CATEGORIES.map(cat => (
            <button key={cat.key} onClick={() => setActiveCategory(cat.key)} style={{ padding: "8px 18px", borderRadius: 999, border: activeCategory === cat.key ? "none" : "1px solid #e8e0d0", background: activeCategory === cat.key ? "#c8873a" : "#fff", color: activeCategory === cat.key ? "#fff" : "#6b4c2a", fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.15s" }}>
              {cat.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 24 }}>
        {filtered.map(cake => <CakeCard key={cake.id} cake={cake} onAddToCart={addToCart} />)}
      </div>
